...
class _WorkshopDetailScreenState extends State<WorkshopDetailScreen> {
          ...
          child: Container(
            ...
            child: StreamBuilder<List<String>>(
              ...
              builder: (context, snapshot) {
                final isWishlisted = snapshot.data?.contains(widget.workshop.id) ?? false;
                return IconButton(
                  icon: Icon(
                  ...
                  onPressed: () async {
                    if (GuestGate.check(context, featureName: 'Wishlist')) {
                      ...
                      if (uid != null) {
                        await FirestoreService().toggleFavorite(uid, widget.workshop.id);
                      } else {
      ...
      flexibleSpace: FlexibleSpaceBar(
        background: Stack(
          children: [
            Positioned.fill(
              child: Image(
                image: widget.workshop.imagePath.trim().startsWith('http')
                    ? NetworkImage(widget.workshop.imagePath.trim())
                    : AssetImage(
                        widget.workshop.imagePath.trim().isNotEmpty
                            ? widget.workshop.imagePath.trim()
        ...
        child: Row(
          ...
          children: [
            ...
            _buildStatItem(
              ...
              widget.workshop.availableSlots.isEmpty
                  ? "No Min"
                  : widget.workshop.availableSlots,
            ),
            Container(width: 1, height: 30, color: Colors.grey[300]),
            _buildStatItem(
              Icons.account_balance_wallet_outlined,
              widget.workshop.pricePerPerson,
            ),
          ],
        ),
      ),
  ...
  Widget _buildOverview() {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Included Materials Card
          if (widget.workshop.includedMaterials.isNotEmpty)
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...widget.workshop.includedMaterials
  ...
  Widget _buildReviews() {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ReviewInputWidget(itemId: widget.workshop.id),
          ReviewListWidget(itemId: widget.workshop.id),
          const SizedBox(height: 32),
          Text(
            "Vibe Checks",
            style: GoogleFonts.poppins(
          ...
          const SizedBox(height: 16),
          VibeCheckWidget(itemId: widget.workshop.id),
          VibeListWidget(itemId: widget.workshop.id),
        ],
      ),
    );
  }
                ...
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (_) => SelectSpotsScreen(
                      ...
                      pricePerPerson: widget.workshop.pricePerPerson,
                      dateRange: 'Flexible',
                      hostName: widget.workshop.hostName,
                    ),
                  ),
                );
              }
  ...
  Widget _buildBottomNav(BuildContext context) {
    return BottomNavigationBar(
      ...
      elevation: 0, // Using elevation 0 so it integrates smoothly below booking bar
